...
Xinix is an Emacs distribution that aims to enhance the default
Emacs experience.  Xinix alters a lot of the default settings,
bundles a plethora of additional packages and adds its own core
library to the mix. The final product offers an easy to use Emacs
configuration for Emacs newcomers and lots of additional power for
Emacs power users.

Xinix is compatible **ONLY with GNU Emacs 24.x**. In general you're
advised to always run Xinix with the latest Emacs - currently
**24.4**.
...
- [Updating Xinix](#updating-xinix)
	- [Manual update](#manual-update)
		...
		- [Update Xinix's code](#update-xinixs-code)
		- [Restart Xinix](#restart-xinix)
	- [Automatic update](#automatic-update)
- [Enabling additional modules](#enabling-additional-modules)
- [Running](#running)
- [Getting to know Xinix](#getting-to-know-xinix)
	- [Keymap](#keymap)
		- [Global](#global)
		- [Xinix Mode](#xinix-mode)
...
```bash
cd path/to/xinix/installation
git pull
```

The `path/to/xinix/installation` is usually `~/.emacs.d` (at least
on Unix systems).

#### Restart Xinix

It's generally a good idea to stop Emacs after you do the update. The
next time Xinix starts it will install any new dependencies (if
there are such).

### Automatic update

Simply run <kbd>M-x xinix-update</kbd> from Emacs itself and restart Emacs afterwards.
...
;; (require 'xinix-haskell)
(require 'xinix-js)
;; (require 'xinix-latex)
(require 'xinix-lisp)
;; (require 'xinix-mediawiki)
(require 'xinix-org)
(require 'xinix-perl)
...
You'll need to adjust your `xinix-modules.el` file once the
installation is done. If you are doing a manual install then you first
need to copy the `xinix-modules.el` available in the sample
directory to the root of `path/to/xinix/installation` and then
adjust that one.
...
## Getting to know Xinix

Certainly the best way to understand how Xinix enhances the default
Emacs experience is to peruse Xinix's source code (which is
obviously written in Emacs Lisp). Understanding the code is not
necessary of course. Xinix includes a `xinix-mode` minor Emacs mode
which collects some of the additional functionality added by
Xinix. It also adds an additional keymap that binds many of those
...
<kbd>C-^</kbd>     | Join two lines into one(`xinix-top-join-line`).
...
<kbd>C-a</kbd>     | Run `xinix-move-beginning-of-line`. Read [this](http://emacsredux.com/blog/2013/05/22/smarter-navigation-to-the-beginning-of-a-line/) for details.
...
Xinix does not mess by default with the standard mapping of `Command` (to `Super`) and `Option` (to `Meta`).
...
You can also temporarily swap them with `C-c w` (`M-x xinix-swap-meta-and-super`).
...
Xinix adds an extra keymap prefix `S-p` (`S` stands for
...
You can learn Helm usage and key bindings following the guide. <kbd>C-c h</kbd> is Xinix's default prefix key for Helm.
If you don't remember any key binding, append <kbd>C-h</kbd> after <kbd>C-c h</kbd> for a list of key bindings in Helm.

If you love Helm and want to use Helm globally with enhanced `helm-find-files`, `helm-buffer-lists`..., you will have to also add `(require 'xinix-helm-everywhere)`.
When `xinix-helm-everywhere` is activated, Helm enables these global key bindings:
...
If you prefer Ido in everywhere, you should not add `xinix-helm-everywhere`, so you can use Helm along with Ido and Xinix's default commands.

You can always reactivate Helm with `(xinix-global-helm-global-mode-on)`.
...
**Key-chords are available only when the `xinix-key-chord` module has been enabled.**
...
<kbd>JJ</kbd>      | Jump back to previous buffer(`xinix-switch-to-previous-buffer`)
...
In some cases you may not want to have a key-chord that is defined by xinix,
...
If you want to use vim inside of emacs enable the `xinix-evil` module which provides
support for `evil-mode`.

## Automatic package installation

The default Xinix installation comes with a bare minimum of
...
a `.clj` file `clojure-mode`, `cider` and Xinix's enhanced Lisp
...
Fork the official Xinix repo and add your own touch to it. You're advised to avoid changing stuff outside of the
...
```lisp
(xinix-require-packages '(some-package some-other-package))
```

If you require just a single package you can also use:

```lisp
(xinix-require-package 'some-package)
```

#### Preloading personal config

Sometimes you might want to load code before Xinix has started loading. Xinix will automatically preload all
Emacs Lisp files in your `personal/preload` directory. Note that at this point you can't using anything from
Xinix, except a few variables like `xinix-dir`, etc (since nothing is yet loaded).
...
xinix-clean-whitespace-on-save to nil in your config file with:

```lisp
(setq xinix-clean-whitespace-on-save nil)
```

The xinix-clean-whitespace-on-save setting can also be set on a
...
updating Xinix, since the latest Xinix code might depend on newer
versions of the bundled packages than you would currently have
installed.

If you're doing manual Xinix updates you should always do a package update first.

`M-x package-list-packages RET U x`

That's not necessary if you're using `M-x xinix-update`, since it
will automatically update the installed packages.

### Problems with flyspell-mode

Xinix makes heavy use of the flyspell-mode package for spell
...
Xinix overrides `C-a` to behave as described
...
Xinix swaps the default `ido` flex matching with the more powerful
